...
Access the data as shown below. These commands initialise an object called **penguins**, which is a **tibble**. A tibble is an enhanced data frame. The additional features will be discussed during the next module. For these exercises you can simply treat object **penguins** as a normal data frame.
...
2. Using base R, generate a histogram showing the distribution of body mass, collapsed across island, species and sex. How would you describe the distribution?

```{r hist, echo=FALSE, eval=FALSE}
hist(penguins$body_mass_g)
```

3. Using ggplot, generate a scatterplot illustrating the relationship between bill length and body mass, collapsed across species, island and sex. Remember to load the library with `library(ggplot2)` before first use. How would you describe the pattern?
...
4. Modify your plot from Exercise 3 so that penguins from the different islands are drawn in different colours. Which island seems to have the heaviest penguins? Without looking any further at the data, formulate at least two possible explanations for the pattern.
...
5. Using ggplot, generate a boxplot comparing body mass for the three different species of penguin, and having each of the three boxes drawn in a different colour. What information is missing from this figure that was provided Exercise 4? What information is easier to see in this figure than in Exercise 4?
...
6. Using ggplot, duplicate this figure. You will need to research geometry function `geom_bar`.
