...
In all of our examples so far, we have used a print statement to show the result of the function's computation. This is, of course, not adequate in real programming, as a print statement simply writes to the console. The computed value is not available to use later in our script. We usually want to be able to save the result of a function into a variable, as we have done with the built-in functions we have used earlier in the course. For example, we might say `x <- sqrt(22)`, creating a variable called *x* that stores the square root of 22. 
...
In R, the `return` keyword is actually optional. By default, R functions just return the value of their last line. So technically, you can write the function above like this without changing its behaviour:
...
calc_BMI <- function(weight, height)
{
  ...
  # Omitting the explicit call to return
  # Don't do this.
  bmi
}
```
