...
Work carefully through each of the sections below, confirming that all your outputs match ours. When you feel completely comfortable with the content, we recommend moving on to Section 3 - Data Visualisation in the  R for Data Science online text (https://r4ds.had.co.nz/data-visualisation.html), which covers advanced plotting with the ggplot library in detail.
...
Each column header is associated with a value **dbl**, or **chr**. These indicate what kind of data R identified in the input file. Columns that are **dbl** are numbers; columns that are **chr** are strings. The same information is shown when RStudio prints the imported data. Beneath each column header you will see **dbl** for numbers and **chr** for strings. 
...
There is a fantastic introduction to Data Visualisation using ggplot in "R for Data Science" (https://r4ds.had.co.nz/data-visualisation.html). It's highly recommended to have a read through this chapter as we'll only touch on a very brief introduction of this framework in this module.
